...
class ApplyUserController extends Controller
{
    public function add(Request $request, $id = Null)
    {
        ...
        if (!empty($coach->stop_apply_weight)) {
            return redirect()->route('coach_applyuser')->withErrors('Tournament apply has been stopped by subadmin.');
        }
        ...
        if ($id != "") {
            $saveurl = url('coach/applyuser/save/' . $id);
            $button = 'Update';
            $page_title = 'Update Athlete';
        } else {
            $saveurl = url('coach/applyuser/save');
            $button = 'Add';
        ...
        return view('coach.applyuser.add')->with($data);
    }
    ...
    public function index()
    {
        ...
        return view('coach.applyuser.view')->with($data);
    }

    public function anydata(Request $request)
    {
        $admin= Auth::guard('vender')->user();
        if (!empty($admin->stop_apply_weight)) {
            return response()->json(['data' => []]);
        }
        $anydata = User::where('status', '!=', 3)->where('coach_id',$admin->id)->where('role', 1)->orderBy('id', 'DESC')->where(function ($query) use ($request) {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('apply', function ($anydata) {
                ...
                $tournaments = Tournament::where('status', 1)
                    ...
                    ->where('gender', $anydata->gender)
                    ->availableForCoachWeight($anydata->district)
                    ->pluck('id')->toArray();
    ...
    public function detail(Request $request, $id)
    {
        ...
        return view('coach.applyuser.detail')->with($data);
    }
