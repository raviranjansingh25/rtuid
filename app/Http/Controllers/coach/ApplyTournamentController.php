...
{
    private function denyIfCoachApplyStopped()
    {
        $coach = Auth::guard('vender')->user();
        if (!empty($coach->stop_apply_weight)) {
            return $coach;
        }
    ...
    public function edit(Request $request, $id = Null)
    {
        if ($this->denyIfCoachApplyStopped()) {
            return redirect()->route('coach_apply_tournament')->withErrors('Tournament apply has been stopped by subadmin.');
        }
        ...
        $saveurl = url('coach/apply-tournament-name_edit_save/' . $id);
        $button = 'Update';
        ...
        return view('coach.applytournament.add')->with($data);
    }
    ...
    public function indexname($id)
    {
        ...
        return view('coach.applytournament.viewname')->with($data);
    }
    ...
    public function anydataname(Request $request,$id)
    {
        ...
        ->whereHas('get_user', function ($q) use ($coach) {
        ...
        ->addColumn('waight_category', function ($anydata) {
            ...
            ->addColumn('id_card', function ($anydata) {
                $file_name = "category";
                $encrypted_id = get_encrypted_value($anydata->id, true);
                $action = '<a class="btn btn-success btn-rounded btn-sm waves-effect waves-light" href="' . url('/coach/single_card/' . $encrypted_id) . '">Download</a>';
                return $action;
            })
            ->addColumn('form', function ($anydata) {
                $file_name = "category";
                $encrypted_id = get_encrypted_value($anydata->id, true);
                $action = '<a class="btn btn-success btn-rounded btn-sm waves-effect waves-light" href="' . url('/coach/reg_form_single/' . $encrypted_id) . '">Download</a> 
            ...
            ->addColumn('action', function ($anydata) {
                ...
                $action = 'N/A';
                if($anydata->final == 0){
                    $action = '<a href="' . url('/coach/apply-tournament-name_edit/' . $encrypted_id) . '"><i class="fas fa-edit" title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a> &nbsp;&nbsp;  
    ...
    {
        if ($this->denyIfCoachApplyStopped()) {
            return response()->json(['status' => 'error', 'message' => 'Tournament apply has been stopped by subadmin.'], 403);
        }
    ...
    {
        if ($this->denyIfCoachApplyStopped()) {
            return response()->json(['status' => 'failed', 'message' => 'Tournament apply has been stopped by subadmin.'], 403);
        }
    ...
    {
        if ($this->denyIfCoachApplyStopped()) {
            return response()->json(['error' => 'Tournament apply has been stopped by subadmin.'], 403);
        }
        ...
        ->pluck('id')->toArray();
            ...
            $tournaments = Tournament::with('get_waight_cat')->where('status', 1)
                ...
                ->availableForCoachWeight($user->district ?? optional($coach)->district)
                ->get();
