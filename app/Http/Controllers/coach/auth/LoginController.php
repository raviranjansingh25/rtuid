...
class LoginController extends Controller
{
    public function index(){
        $data = array(
            'saveurl' =>url('/coach/login-save'),
            'title' =>"Coach Login",
        );
        return view('coach.auth/login')->with($data);
    }
    ...
    public function save(Request $request) {
      ...
      $admin = Coach::where('email',$email)->first();
      // p($admin);

      if ($admin!="") {
       
          
          if(Auth::guard('vender')->attempt(['email' => $email, 'password' => $password])) {
            
            return redirect('/coach/dashboard')->withSuccess("You are successfully logged in");
          }
          else{
    ...
    public function change_password(){
      ...
      return view('coach.auth.change_password')->with($data);
    }
    ...
    public function change_password_save(Request $request)
    {
       ...
       $adminDetails = Coach::where(['id' => $admin_id])->first();
       ...
       if(Hash::check($request->get('current-password'), $adminDetails['password'])){
        ...
        if ($validatior->fails())
        {
            ...
            Coach::where('id', $admin_id)->update($values);
