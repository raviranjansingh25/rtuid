...

{
...

    {
        ...
        if ($id != "") {
            ...
            $saveurl = url('coach/user/save/' . $id);
        ...
        return view('coach.user.add')->with($data);

    }

    public function updateItUid(Request $request)
{
    ...
    if ($user->edit_uid != 1) {
        return response()->json([
            'status' => 'error',
            'message' => 'Permission denied for updating IT UID!'
        ], 403);
    }

    $user->it_uid = $request->it_uid;
    $user->edit_uid = 0; // Update hote hi edit_uid ko 1 se 0 kar diya gaya hai
    $user->save();

    return response()->json([
        'status' => 'success',
        'message' => 'IT UID Updated Successfully!'
    ]);
}
...

    {
        ...
        return redirect()->route('coach_user')->withSuccess($success_msg);
...

    {
        ...
        return view('coach.user.view')->with($data);
   ...
   public function anydata(Request $request)
{
    ...
    return Datatables::of($anydata)
        ...
        // Pehla Alag Column: Edit IT UID Button
        ->addColumn('edit_ituid_btn', function ($anydata) {
            if ($anydata->edit_uid == 1) {
                return '<button type="button" class="btn btn-warning btn-sm edit-ituid-btn" data-id="' . $anydata->id . '" data-ituid="' . ($anydata->it_uid ?? '') . '" title="Edit IT UID"><i class="fas fa-edit"></i> Edit IT UID</button>';
            } else {
                return '';
            }
        })
        ...
        ->addColumn('district_name', function ($anydata) {
            return $anydata->district ?? 'N/A';
        })

        ->addColumn('code_id', function ($anydata) {
            return $anydata->code_id ?? 'N/A';
        })

        ->addColumn('it_uid', function ($anydata) {
            return $anydata->it_uid ?? 'N/A';
        })

        ->addColumn('user_gender', function ($anydata) {
            $genders = [1 => 'Male', 2 => 'Female', 3 => 'Other'];
            return $genders[$anydata->gender] ?? 'N/A';
        })
        ...
        ->addColumn('action', function ($anydata) {
            ...
            if ($anydata->coach_act == 2 || $anydata->status == 4) {
                $act = '<a href="' . url('/coach/user/add/' . $encrypted_id) . '"><i class="fas fa-edit" title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a>';
            } else {
                $act = '';
            }

            $action = '<a href="' . url('/coach/user/detail/' . $encrypted_id) . '"><i class="mdi mdi-eye text-info" title="View" data-toggle="tooltip" data-placement="bottom"></i></a>&nbsp;&nbsp;' . $act;
            return $action;
        })

        // edit_ituid_btn ko rawColumns me shamil karna zaroori hai
        ->rawColumns(['edit_ituid_btn', 'status', 'action', 'state_data', 'profile', 'total_referral', 'wallet', 'phone', 'name', 'waight_button', 'apply'])
...

    {
        ...
        return view('coach.user.detail')->with($data);
