...
class LoginController extends Controller
{
    public function index(){
        $data = array(
            'saveurl' =>url('/subadmin/login-save'),
            'title' =>"Subadmin Login",
        );
        return view('subadmin.auth.login')->with($data);
    }
    ...
    public function save(Request $request) {
      ...
      $admin = Admin::where('email',$email)->where('role', 2)->first();

      if ($admin!="") {
          if(Auth::guard('subadmin')->attempt(['email' => $email, 'password' => $password, 'role' => 2])) {
            return redirect('/subadmin/dashboard')->withSuccess("You are successfully logged in");
          }
          else{
    ...
    public function change_password(){
      $data = array(
        ...
        'saveurl' =>route('subadmin_change_password_save'), 
      );
      return view('subadmin.auth.change_password')->with($data);
    }

    public function change_password_save(Request $request)
    {
       $admin_id = Auth::guard('subadmin')->user()->id;
    ...
    public function view_profile(){
      $profile= Auth::guard('subadmin')->user();
      $data = array(
        'title' =>'Update Profile', 
        'saveurl' =>route('subadmin_update_profile'), 
        'profile' => $profile,
      );
      return view('subadmin.auth.profile_updeate')->with($data);
    }
