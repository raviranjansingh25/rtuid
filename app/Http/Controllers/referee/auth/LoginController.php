...
namespace App\Http\Controllers\referee\auth;
...
class LoginController extends Controller
{
    public function index(){
        $data = array(
            'saveurl' =>url('/referee/login-save'),
            'title' =>"Referee Login",
        );
        return view('referee.auth/login')->with($data);
    }
    ...
    public function save(Request $request) {
      ...
      $admin = Referee::where('email',$email)->first();
      // p($admin);

      if ($admin!="") {
       
          
          if(Auth::guard('referee')->attempt(['email' => $email, 'password' => $password])) {
            
            return redirect('/referee/dashboard')->withSuccess("You are successfully logged in");
          }
          else{
    ...
    public function change_password(){
      ...
      return view('referee.auth.change_password')->with($data);
    }

    public function change_password_save(Request $request)
    {
       $admin_id = Auth::guard('referee')->user()->id;
       
       $adminDetails = Referee::where(['id' => $admin_id])->first();
       ...
       if(Hash::check($request->get('current-password'), $adminDetails['password'])){
        ...
        if ($validatior->fails())
        {
          ...
          Referee::where('id', $admin_id)->update($values);
    ...
    public function view_profile(){
      $profile= Auth::guard('referee')->user();
      $data = array(
        'title' =>'Update Profile', 
        'saveurl' =>route('referee_update_profile'), 
        'profile' => $profile,
      );
      return view('referee.auth.profile_updeate')->with($data);
    }
    ...
    public function logout()
    {
        ...
        return redirect()->route('refereelogin');
    }
}
