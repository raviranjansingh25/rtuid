...
<div class="col-md-12 col-lg-9 complete-profile-000">
  <div class="row">
      ...
      <div class="profile-dr">
        ...
        <div class="detail-dr">
          ...
          <h5><span class="d-block mb-2">Qualifications</span>
            <?php
            $data = [];
            foreach ($user['multi_qulafication'] as $value) {

              $title = isset($value['get_quali']->title) ? $value['get_quali']->title : 'N/A';
              $year = isset($value->year_graduated) ? $value->year_graduated : 'N/A';
              $title . ' in ' . $year;
            ?>
              {{$title}} in {{$year}}<br>
            <?php
        ...
        <div class="profile-action-btn text-center">
          ...
          <form method="post" action="{{route('account_update')}}">
            ...
            <button type="submit" class="btn-primary w-100">Add Bank Details</button>
            @endif
          </form>
        </div>
      </div>
    ...
    <div class="col-xl-4 col-lg-4 col-md-4">
      <div class="all-consultations-date">
        <h3 class="text-start mt-0">Meet {{$user->name}}</h3>
