...
<div id="contentToPrint">
  @foreach($get_data as $event)
	...
	<!--<div class="main">-->
		...
		<div class="th19">
			<h5>({{$event['get_newevent']->title}})</h5>
		</div>

		<div class="img-border">
			<img src="{{url($event['get_user']->passport)}}" width="130" height="130">

		</div>
		@php
		if($event['get_user']->gender==1){ 
		    $gender = 'Male'; 
	   }
	 elseif($event['get_user']->gender==2){
	     $gender = 'Female'; 
    }
    else {
        $gender = 'Transgender';
...
<div class="fill-row ">
	<div class="row-box">
		...
		<div class="register-number">{{$event['get_user']->code}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">STATE</div>
		<div class="register-number">{{$event['get_user']['get_state']->name}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">1. Player's Name</div>
		<div class="register-number" style="width:150px;">{{$event['get_user']->name}} {{$event['get_user']->middlename}} {{$event['get_user']->lastname}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">2. Fathers's Name</div>
		<div class="register-number" style="width:150px;">{{$event['get_user']->fathername}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">3. Date of Birth</div>
		<div class="register-number">{{$event['get_user']->dob}}</div>
	</div>
	<div class="row-box">
	...
	<div class="row-box">
		...
		<div class="register-number">{{$event['get_user']['get_cat']->name}}</div>
	</div>
	<div class="row-box">
	...
	<div class="row-box">
		...
		<div class="register-number">{{$event['get_user']->mobile}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">8. Email Id.</div>
		<div class="register-number">{{$event['get_user']->email}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">9. Address</div>
		<div class="register-number" style="width: 70%;
		">{{$event['get_user']->street}}, {{$event['get_user']->city}}</div>
	</div>
	<div class="row-box">
		<div class="reg boldreg">10. Participating</div>
		<div class="register-number">
      @php
      $start_date = $event['get_newevent']->start_date;
      $event1 = App\Models\GroupEventApplied::where('applied_id',$event->applied_id)->whereHas('get_newevent', function($q) use($start_date) {
        
                    if (!empty($start_date) ) 
                    {
                       $q->where('start_date', '=', $start_date);
                    }
                })->with('get_newevent')->get();
                
        $event2 = App\Models\EventApplied::where('applied_id',$event->applied_id)->whereHas('get_newevent', function($q1) use($start_date) {
            if (!empty($start_date) ) 
                    {
