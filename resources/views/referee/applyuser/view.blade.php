...
<div class="modal fade" id="applyTournamentModal" tabindex="-1" aria-labelledby="applyModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    pageLength: 25,
                 ajax: {
                     url: "{{ url('/referee/applyuser-data') }}",
                     data: function(d) {
             ...
             $(document).on("click", ".delete-button", function(e) {
                 ...
                 swal({
                     ...
                     if (willDelete) {
                         $.ajax({
                             url: "{{ url('referee/applyuser/delete') }}",
                             datatType: 'json',
                             type: 'GET',
                             data: {
                                 'id': id,
     ...
     <script type="text/javascript">
         function changeStatus(id, status) {
             ...
             $.ajax({
                 url: "{{ url('referee/applyuser/status') }}",
                 type: 'GET',
                 data: {
                     'id': id,
                     'status': status,
...
<script>
  ...
  function submitWeight() {
    ...
    $.ajax({
      url: '{{ url("referee/apply_update-weight") }}', // your route
      method: 'POST',
      data: {
        _token: '{{ csrf_token() }}',
        user_id: userId,
...
<script>
  function apply(userId) {
    ...
    $.ajax({
      url: '{{ url("referee/get-tournaments") }}' + '?user_id=' + userId, // get Tournament data
      type: 'GET',
  ...
  function submitTournament() {
    ...
    $.ajax({
      url: '{{ url("referee/data/apply-tournament") }}',
      method: 'POST',
      data: {
        _token: '{{ csrf_token() }}',
        user_id: userId,
