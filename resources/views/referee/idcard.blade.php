...
<style>
    ...
    .front-card {
        background-image: url("{{ asset('public/referee_card_img2.jpg') }}");
    }

    .back-card {
        background-image: url("{{ asset('public/referee_card_img1.jpg') }}");
    }
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item"><a href="{{ route('referee_dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item active">Referee ID Card</li>
                            </ol>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="cards-container" id="printable-cards-area">
                            ...
                            <div class="card-wrapper">
                                <div class="id-card-frame front-card" id="card-front">
                                    ...
                                    <div class="card-value referee-name">{{ $referee->name }}</div>
                                    <div class="card-value referee-id">RF-{{ sprintf('%05d', $referee->id) }}</div>
                                    <div class="card-value father-name">-</div>
                                    <div class="card-value dob-val">-</div>
                                    <div class="card-value grade-val">-</div>
                                    
                                    <div class="card-value join-date">{{ \Carbon\Carbon::parse($referee->created_at)->format('d/m/Y') }}</div>
                                    <div class="card-value expired-date">{{ \Carbon\Carbon::parse($referee->created_at)->addYears(2)->format('d/m/Y') }}</div>
                                    
                                    <!-- Photo container -->
                                    <div class="photo-container">
                                        @if(!empty($referee->image) && file_exists(public_path($referee->image)))
                                            <img src="{{ asset($referee->image) }}" class="referee-photo" alt="Photo">
                                        @elseif(!empty($referee->image))
                                            <img src="{{ url($referee->image) }}" class="referee-photo" alt="Photo">
                                        @else
                                            <img src="{{ url('public/noimage.png') }}" class="referee-photo" alt="Photo">
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="id-card-frame back-card" id="card-back">
                                    ...
                                    <div class="card-value mobile-num">
                                        @if(!empty($referee->phone))
                                            +{{ $referee->country_code ?? '91' }} {{ $referee->phone }}
                                        @else
                                            -
                                        @endif
                                    </div>
...
<script>
    async function downloadCardPDF() {
        ...
        try {
            ...
            // Save the PDF
            pdf.save("Referee_ID_Card_{{ $referee->name }}.pdf");
        } catch (error) {
