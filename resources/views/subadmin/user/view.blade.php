             ...
             <div class="row mb-3">
                 <div class="col-12">
                     <div class="alert alert-info mb-0">
                         ...
                         @if($permissions['athlete_detail']) <span class="badge bg-success ms-2">Detail</span> @endif
                         @if(!$canAccessAllDistricts && $permissions['athlete_edit']) <span class="badge bg-primary ms-1">Edit</span> @endif
                     </div>
                 </div>
             </div>
             <div class="row">
                                 ...
                                 <div class="col-12">
                                     ...
                                     <div class="d-sm-flex align-items-center justify-content-between">
                                         ...
                                         <div class="page-title-right add_button">
                                             <a href="{{ route('subadmin_user_add') }}">
                                                 <button type="button" class="btn btn-success waves-effect waves-light">Add Athlete</button>
                                             </a>
                                         </div>
                                         @endif
                                     </div>
                             ...
                             <table class="table table-bordered table_data mb-0 table-bordered">
                                 <thead>
                                     <tr>
                                        ...
                                        @endif
                                         <th>Status</th>
                                         @if($permissions['athlete_detail'] || ($canAccessAllDistricts && ($permissions['athlete_detail'] || $permissions['athlete_edit'])) || !$canAccessAllDistricts)
  ...
  <div class="modal-dialog">
    <div class="modal-content">
     ...
     <script>
         $(document).ready(function() {
             var table = $('.table_data').DataTable({
                 ...
                 @if($canAccessAllDistricts || !$canAccessAllDistricts)
                 dom: 'Bfrtip',
                buttons: [
                {
                    extend: 'excelHtml5',
                 ...
                 columns: [
                    ...
                    @endif
                     ...
                     @if($permissions['athlete_detail'] || $canAccessAllDistricts || !$canAccessAllDistricts)
                     {
                         data: 'action',
                         name: 'action'
                     }
     ...
     <script type="text/javascript">
         function verifyAthlete(id) {
             $.ajax({
                 url: "{{ url('subadmin/user/verify') }}",
                 type: 'POST',
                 data: {
                     _token: '{{ csrf_token() }}',
                     id: id
                 ...
                 success: function(res) {
                     if (res.status === 'success') {
                         ...
                         swal("Error", res.message || "Verification failed.", "error");
                     }
                 },
                 error: function() {
...
<script>
  ...
  function submitWeight() {
    ...
    $.ajax({
      url: '{{ url("subadmin/update-weight") }}', // your route
      method: 'POST',
      data: {
        _token: '{{ csrf_token() }}',
        user_id: userId,
...
<script>
  function apply(userId) {
    ...
    $.ajax({
      url: '{{ url("subadmin/get-tournaments") }}' + '?user_id=' + userId, // get Tournament data
      type: 'GET',
  ...
  function submitTournament() {
    ...
    $.ajax({
      url: '{{ url("subadmin/data/apply-tournament") }}',
      method: 'POST',
      data: {
        _token: '{{ csrf_token() }}',
        user_id: userId,
