             ...
             <div class="row mb-3">
                 <div class="col-12">
                     <div class="alert alert-info mb-0">
                         ...
                         @if($permissions['add_weight']) <span class="badge bg-success ms-2">Add Weight</span> @endif
                         @if($permissions['can_apply']) <span class="badge bg-primary ms-1">Apply</span> @endif
                     </div>
                 </div>
             </div>
             <div class="row">
...
<div class="modal fade" id="applyTournamentModal" tabindex="-1" aria-labelledby="applyModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    pageLength: 25,
                 ajax: {
                     url: "{{ url('/subadmin/applyuser-data') }}",
                     data: function(d) {
             ...
             $(document).on("click", ".delete-button", function(e) {
                 ...
                 swal({
                     ...
                     if (willDelete) {
                         $.ajax({
                             url: "{{ url('subadmin/applyuser/delete') }}",
                             datatType: 'json',
                             type: 'GET',
                             data: {
                                 'id': id,
     ...
     <script type="text/javascript">
         function changeStatus(id, status) {
             ...
             $.ajax({
                 url: "{{ url('subadmin/applyuser/status') }}",
                 type: 'GET',
                 data: {
                     'id': id,
                     'status': status,
...
<script>
  ...
  function submitWeight() {
    ...
    $.ajax({
      url: '{{ url("subadmin/apply_update-weight") }}', // your route
      method: 'POST',
      data: {
        _token: '{{ csrf_token() }}',
        user_id: userId,
