...
<style>
    ...
    .front-card {
        background-image: url("{{ asset('public/subadmin_card_img2.jpg') }}");
    }

    .back-card {
        background-image: url("{{ asset('public/subadmin_card_img1.jpg') }}");
    }
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item active">Subadmin ID Card</li>
                            </ol>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="cards-container" id="printable-cards-area">
                            ...
                            <div class="card-wrapper">
                                <div class="id-card-frame front-card" id="card-front">
                                    ...
                                    <div class="card-value subadmin-name">{{ $subadmin->name }}</div>
                                    <div class="card-value subadmin-id">RF-{{ sprintf('%05d', $subadmin->id) }}</div>
                                    <div class="card-value father-name">-</div>
                                    <div class="card-value dob-val">-</div>
                                    <div class="card-value grade-val">-</div>
                                    
                                    <div class="card-value join-date">{{ \Carbon\Carbon::parse($subadmin->created_at)->format('d/m/Y') }}</div>
                                    <div class="card-value expired-date">{{ \Carbon\Carbon::parse($subadmin->created_at)->addYears(2)->format('d/m/Y') }}</div>
                                    
                                    <!-- Photo container -->
                                    <div class="photo-container">
                                        @if(!empty($subadmin->image) && file_exists(public_path($subadmin->image)))
                                            <img src="{{ asset($subadmin->image) }}" class="subadmin-photo" alt="Photo">
                                        @elseif(!empty($subadmin->image))
                                            <img src="{{ url($subadmin->image) }}" class="subadmin-photo" alt="Photo">
                                        @else
                                            <img src="{{ url('public/noimage.png') }}" class="subadmin-photo" alt="Photo">
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-wrapper">
                                <div class="id-card-frame back-card" id="card-back">
                                    ...
                                    <div class="card-value mobile-num">
                                        @if(!empty($subadmin->phone))
                                            +{{ $subadmin->country_code ?? '91' }} {{ $subadmin->phone }}
                                        @else
                                            -
                                        @endif
                                    </div>
...
<script>
    async function downloadCardPDF() {
        ...
        try {
            ...
            // Save the PDF
            pdf.save("Subadmin_ID_Card_{{ $subadmin->name }}.pdf");
        } catch (error) {
